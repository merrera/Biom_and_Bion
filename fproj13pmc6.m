function BS2050kstp = fproj13pmc6 ( params ); 
% Projecao de cenarios Bioenergia
% Artigo Tomas_et_al 2
% GEANEX/UFPR
% Criado em 24/fev/2022
...
BS2050kstp = zeros (N_BS, N_S); % bioenergia [EJ] 2050
% inicio de variaveis
...
if nargin < 1,
 ...
 % vr3 acima definido como zero
  ...
  %
  eta_ref (1)  = 0.24; % firewood
  eta_ref (2)  = 0.24; % charcoal
  eta_ref (3)  = 0.24; % residuos florestais
  eta_ref (4)  = 0.18; % livestock wastes
  eta_ref (5)  = 0.25; % agricultural wastes
  eta_ref (6)  = 0.25; % energy crops
  eta_ref (7)  = 0.32; % black liquor 
  eta_ref (8)  = 0.24; % wood industry
  eta_ref (9)  = 0.24; % recovered wood
  eta_ref (10) = 0.18; % MSW
  %
  vr4 (1:3,3) = 0.25; % OPT firewood, charcoal & forest residues
  vr4 (1:3,4) = 0.5;  % FAR firewood, charcoal & forest residues
  vr4 (4,3) = 0.6; % OPT firewood, charcoal & forest residues
  vr4 (4,4) = 1.2; % FAR firewood, charcoal & forest residues
  vr4 (5:6,3) = 0.14; % OPT agri wastes & energy crops
  vr4 (5:6,4) = 0.28; % FAR agri wastes & energy crops
  vr4 (7,3) = 0.2; % OPT black liquor
  vr4 (7,4) = 0.4; % FAR black liquor
  vr4 (8:9,3) = 0.25; % OPT wood ind res & recov wood
  vr4 (8:9,4) = 0.5;  % FAR wood ind res & recov wood 
  vr4 (10,3) = 0.6; % OPT MSW
  vr4 (10,4) = 1.2; % FAR MSW   
else
    BSG_CS = params(1);
    pEFLA = params(2);
...
%%
%%    ALGO DE ERRADO COM Vr1 ????? Já corrigiu ???
%%
...
for j = 1:N_BS,
    for i = 1:N_S,
     BS2050kstp (j,i) = BS_CS (j) * vr1(j,i) * ( 1 + vr2(j,i) )* ( 1 + vr3 (j,i)); % projecoes Bioenergia 2050 [EJ]
     BS2050kstp (j,i) = BS2050kstp (j,i) * eta_ref (j)*( 1 + vr4(j,i));
    end; % j
end; % i

BS2050stp = sum(BS2050kstp); 
save ('proj13mc6BS2050pmean.txt', 'BS2050stp', '-ascii');
save ('proj13mc6p.txt', 'BS2050kstp', '-ascii');
%%